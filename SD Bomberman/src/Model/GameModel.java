...
public class GameModel {
	...
	public GameModel(int scX,int scY,int playerNum) {
		/**Le modele charge, entre autres, une liste de cases a eviter lors
		 * du placement des murs cassables, et les coordonnees des limites
		 * du tableau, ou des murs incassables doivent etre places.
		 */
	...
	public void layWalls(){
		 ...
		 * MURS SOLIDES: cases aux deux coordonees impaires, et bord du tableau.
	...
	public void powerUpCheck(){
		/**Gere les collisions des joueurs (en surface) avec les powerups.
	...
	public void hitPlayersCheck(){
		 ...
		 * 1) Si le joueur est mort, on declenche une animation (pose de mort).
	...
	public void hitWallsCheck(){
		/**Enleve les murs casses et met eventuellement un powerup
		 * aleatoire a leur place.
		 */
		for(int i=0; i<walls.size(); i++){
			Wall w = walls.get(i);
	...
	public void powerUpInWallCheck(ArrayList<PowerUp> powList){
		/**Cherche les eventuels murs cassables sur desquels il y a
		 * des powerups, et les enleve.
		 */
	...
	public void holeCheck(){
		/**Enleve les trous de Bombe Taupe apres un temps.
		 */
			...
			if(exp.getBurnedOut()){
				...
				if(!bit.getOnEnd()){ //Pour que le morceau casse des objets, il ne peut pas etre au bout.
					for(int k=0; k<walls.size(); k++){
						Wall w = walls.get(k);
						if(w.getBreaking() == 0 && breakCheck(w, bit.getPosX(), bit.getPosY(), bit.getDir())){w.breakTimer();}
	...
	public void endGameCheck(){
		/**Se charge de finir le jeu lorsque au plus un joueur reste en vie.
		 */
		int checkWinner = 0;
		over = true;
